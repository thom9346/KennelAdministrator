        ...
        stage("Deploy to Docker Swarm") {
            steps {
               // Update the kennel-db service
                ...
                // Update the kennelapi service
                bat "docker service update kennelapi --image thohol02/kennelapi-backend:$BUILD_NUMBER || docker service create --name kennelapi --env ASPNETCORE_ENVIRONMENT=Development --env ASPNETCORE_URLS=http://+:80 --env ConnectionStrings__MainDB=Server=kennel-db;Database=KennelDB;User Id=sa;Password=8zWt4!LmR6@kC3#eQb0;TrustServerCertificate=true;Connection Timeout=30 --publish 5000:80 --replicas 1 thohol02/kennelapi-backend:$BUILD_NUMBER"
